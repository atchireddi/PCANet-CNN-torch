...
-- options contains
function Classifier:__init(options)
    ...
    if options.model == 'SVM' then
        ...
        self.criterion = nn.ClassNLLCriterion() -- Negative log-likelihood criterion.
    else if options.model == 'CNN' then
        -- self.net:add(nn.Linear(nInputDim, 10)):add(nn.LogSoftMax())
    end
end
